...
static void cpu_sample(void) {
  ...
  while (fscanf(f, "%15s %llu %llu %llu %llu %llu %llu %llu %llu", id, &u, &n,
    ...
    if (initialized) {
      if (idx == 0) {
        cpu.total = use; // Fixed: removed division by NCPU
  ...
  while ((e = readdir(d)) && nprocs < MAX_PROCS) {
    ...
    for (int i = 0; i < prevn; i++) {
      if (prev[i].pid == pid) {
        ...
        // Fixed: Calculate CPU percentage based on wall-clock time
...
static bool is_system_critical(const char *comm) {
  // Never suspend these critical system processes
...
static void draw_graphs(void) {
  ...
  snprintf(tlabel, sizeof(tlabel), "%s",
           tz_path[0]
               ? (strrchr(tz_path, '/') ? strrchr(tz_path, '/') + 1 : "Sensor")
               : "N/A");
